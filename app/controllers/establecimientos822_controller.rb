# -*- encoding : utf-8 -*-
class Establecimientos822Controller < ApplicationController
  ...
  def lista
    ...
    if params[:form_buscar_establecimientos_reparacion_comedor].present?

      cond << "r_comedor #{params[:form_buscar_establecimientos_reparacion_comedor_operador]} ?"
      args << params[:form_buscar_establecimientos_reparacion_comedor]

    end

    if params[:form_buscar_establecimientos_obras_exteriores_pozo].present?

      cond << "oe_pozo #{params[:form_buscar_establecimientos_obras_exteriores_pozo_operador]} ?"
      args << params[:form_buscar_establecimientos_obras_exteriores_pozo]

    end

    if params[:form_buscar_establecimientos_obras_exteriores_indigenas].present?

      cond << "oe_escindi #{params[:form_buscar_establecimientos_obras_exteriores_indigenas_operador]} ?"
      args << params[:form_buscar_establecimientos_obras_exteriores_indigenas]
    ...
    if params[:ordenacion_columna].present? && params[:ordenacion_direccion].present?
      @establecimientos = Establecimiento822.order(params[:ordenacion_columna] + " " + params[:ordenacion_direccion]).where(cond).paginate(page: params[:page], per_page: 15)
    else
      @establecimientos = Establecimiento822.orden_dep_dis.where(cond).paginate(page: params[:page], per_page: 15)
    end

    @total_registros = Establecimiento822.count 
    ...
    if params[:format] == 'csv'
      ...
      if params[:ordenacion_columna].present? && params[:ordenacion_direccion].present?
        establecimientos_csv = Establecimiento822.order(params[:ordenacion_columna] + " " + params[:ordenacion_direccion]).where(cond)
      else
        establecimientos_csv = Establecimiento822.orden_dep_dis.where(cond)
      end
      ...
      csv = CSV.generate do |csv|
        ...
        csv << ["anio", "codigo_establecimiento", "codigo_departamento", "nombre_departamento", "codigo_distrito", "nombre_distrito", "codigo_zona", "nombre_zona", "codigo_barrio_localidad",
          ...
          "r_aulas", "r_sshh", "oe_pozo", "cercado", "oe_escindi" ]
        ...
        establecimientos_csv.each do |e|
          csv << [e.anio, e.codigo_establecimiento, "#{e.codigo_departamento}", e.nombre_departamento, e.codigo_distrito, e.nombre_distrito, e.codigo_zona, e.nombre_zona, e.codigo_barrio_localidad,
            ...
            e.r_aulas, e.r_sshh, e.oe_pozo, e.cercado, e.oe_escindi]
        end

      end
    
      send_data(csv, :type => 'text/csv', :filename => "establecimientos822_#{Time.now.strftime('%Y%m%d')}.csv")

    elsif params[:format] == 'xlsx'
      
      if params[:ordenacion_columna].present? && params[:ordenacion_direccion].present?
        establecimientos_xlsx = Establecimiento822.order(params[:ordenacion_columna] + " " + params[:ordenacion_direccion]).where(cond)
      else
        establecimientos_xlsx = Establecimiento822.orden_dep_dis.where(cond)
      end
       
      p = Axlsx::Package.new
        
      p.workbook.add_worksheet(:name => "Establecimientos822") do |sheet|
        ...
        establecimientos_xlsx.each do |e|
          ...
          sheet.add_row [e.anio, e.codigo_establecimiento, "#{e.codigo_departamento}", e.nombre_departamento, e.codigo_distrito, e.nombre_distrito, e.codigo_zona, e.nombre_zona, e.codigo_barrio_localidad,
            e.nombre_barrio_localidad, e.direccion, e.latitud, e.longitud, e.uri, e.asentamientos_priorizados, e.a_comedor, e.a_aulas, e.a_aula_de_pe, e.a_cra, e.a_lab, e.a_bloq_adm,
            e.r_aulas, e.r_sshh, e.oe_pozo, e.cercado, e.oe_escindi]
      ...
      send_data p.to_stream.read, filename: "establecimientos822_#{Time.now.strftime('%d%m%Y__%H%M')}.xlsx", :type => "application/vnd.openxmlformates-officedocument.spreadsheetml.sheet", disposition: 'attachment'
      ...
      if params[:ordenacion_columna].present? && params[:ordenacion_direccion].present?
        establecimientos = Establecimiento822.order(params[:ordenacion_columna] + " " + params[:ordenacion_direccion]).where(cond)
      else
        establecimientos = Establecimiento822.orden_dep_dis.where(cond)
      end
      ...
      send_data report.generate, filename: "establecimientos822_#{Time.now.strftime('%d%m%Y__%H%M')}.pdf", 
      ...
      if params[:ordenacion_columna].present? && params[:ordenacion_direccion].present?
        establecimientos822_json = Establecimiento822.order(params[:ordenacion_columna] + " " + params[:ordenacion_direccion]).where(cond)
      else
  ...
  def ubicaciones_geograficas
    ...
    #@establecimientos = cond.size > 0 ? ( Establecimiento822.orden_dep_dis.paginate :conditions => cond, 
    #                                                                           :per_page => 15,
    #                                                                           :page => params[:page]) : {}
  ...
  def establecimientos_doc
    ...
    @establecimiento = Establecimiento822.find_by_codigo_establecimiento_and_anio(params[:codigo_establecimiento], anio)
