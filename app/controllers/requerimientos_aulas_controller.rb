class RequerimientosAulasController < ApplicationController
  ...
  def diccionario
    ...
    @diccionario_requerimientos_aulas = clean_json(diccionario)

    if params[:format] == 'json'
      
      generate_json_table_schema(@diccionario_requerimientos_aulas)

    elsif params[:format] == 'pdf'
      
      send_data(generate_pdf(@diccionario_requerimientos_aulas, params[:nombre]), :filename => "diccionario_requerimientos_aulas.pdf", :type => "application/pdf")
  ...
  def lista
    ...
    if params[:form_buscar_requerimientos_aulas][:nombre_zona].present?

      cond << "nombre_zona = ?"
      args << "#{params[:form_buscar_requerimientos_aulas][:nombre_zona]}"
    ...
    if params[:format] == 'csv'
      ...
      if params[:ordenacion_columna].present? && params[:ordenacion_direccion].present?
        requerimientos_aulas_csv = VRequerimientoAula.order(params[:ordenacion_columna] + " " + params[:ordenacion_direccion]).where(cond).all
      else
      ...
      if params[:ordenacion_columna].present? && params[:ordenacion_direccion].present?
        requerimientos_aulas_xlsx = VRequerimientoAula.order(params[:ordenacion_columna] + " " + params[:ordenacion_direccion]).where(cond).all
      else
      ...
      if params[:ordenacion_columna].present? && params[:ordenacion_direccion].present?
        @requerimientos_aulas_todos = VRequerimientoAula.order(params[:ordenacion_columna] + " " + params[:ordenacion_direccion]).where(cond).all
      else
