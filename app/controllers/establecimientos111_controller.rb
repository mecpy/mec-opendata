...
class Establecimientos111Controller < ApplicationController
  ...
  def diccionario
    ...
    require 'json'
    file = File.read("#{Rails.root}/app/assets/javascripts/diccionario/establecimientos111.json")
    diccionario = JSON.parse(file)
    @diccionario_establecimientos111 = clean_json(diccionario)

    if params[:format] == 'json'
      
      generate_json_table_schema(@diccionario_establecimientos111)

    elsif params[:format] == 'pdf'
      
      send_data(generate_pdf(@diccionario_establecimientos111, params[:nombre]), :filename => "diccionario_establecimientos111.pdf", :type => "application/pdf")
  ...
  def lista
    ...
    if params[:format] == 'csv'
      ...
      send_data(csv, :type => 'text/csv', :filename => "establecimientos111_#{Time.now.strftime('%Y%m%d')}.csv")
      ...
      p.workbook.add_worksheet(:name => "Establecimientos111") do |sheet|
      ...
      send_data p.to_stream.read, filename: "establecimientos111_#{Time.now.strftime('%d%m%Y__%H%M')}.xlsx", :type => "application/vnd.openxmlformates-officedocument.spreadsheetml.sheet", disposition: 'attachment'
      ...
      send_data report.generate, filename: "establecimientos111_#{Time.now.strftime('%d%m%Y__%H%M')}.pdf", 
      ...
      respond_to do |f|
        ...
        f.js
        f.json {render json: establecimientos111_json, methods: :uri }

      end 
        
    else
      
      @establecimientos_todos = Establecimiento111.orden_dep_dis.where(cond)
